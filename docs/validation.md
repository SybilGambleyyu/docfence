...
DocFence 0.61 is validated as a parser and reporting boundary, not as a Word
...
- OPC package digital-signature material is separately inventoried from generic
  ...
  canonicalization form, plus a malformed Relationship Transform and
  empty/duplicate transform lists,
  ...
  ASCII-case-insensitive ID/type selector matching and case-colliding ID
  ...
  Transform with a wrong relationships content type or naming a Word part, and
  duplicate Relationship Transforms for one declared relationships part,
  duplicate direct `ds:Transforms` lists, direct part references with permitted
  C14N-only and unsupported empty/duplicate lists, missing or
...
dependencies. For 0.61, the full public corpus contains 38 Relationship
...
The corpus has 152 package-manifest references; none of its 133 resolvable
references requires a case-only ContentType match. Public profiles are
byte-for-byte identical to 0.60.
...
only, not field evaluation. Those limits are explicit in the 0.61 contract; see
