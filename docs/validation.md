...
DocFence 0.6 is validated as a parser and reporting boundary, not as a Word
...
- volatile Word `rsid` updates and relationship-ID renumbering that preserve the
  ...
  mail-merge state, and data-binding storage association remain quiet;
- generated JSON, Markdown, and SARIF do not reproduce unique markers placed
  ...
  macros, embedded/control payloads, alternative-format imports, or opaque
  package parts;
...
- DTD/entity markup, unsafe ZIP member names, and `w:altChunk` markup without a
  ...
  recognized data-binding custom-XML-properties relationships are rejected
  before reporting.
...
from the OOXML Reference Corpus. Those tools and fixtures are not DocFence
...
renderer. Those limits are explicit in the 0.6 contract; see the
